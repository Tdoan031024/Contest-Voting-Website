...
export function PageHeader({ category, title, description, action }: PageHeaderProps) {
  return (
    <div className="admin-card flex flex-col gap-4 px-6 py-6 md:flex-row md:items-end md:justify-between select-none">
      <div>
        <p className="text-[10px] font-black uppercase tracking-[0.22em] text-[var(--primary-strong)]">{category}</p>
        <h2 className="mt-2 text-[28px] font-extrabold tracking-[-0.03em] text-slate-950">{title}</h2>
...
export function SectionHeader({ title, subtitle, action }: { title: string; subtitle?: string; action?: React.ReactNode }) {
  return (
    <div className="mb-5 flex items-end justify-between border-b border-slate-100 pb-4 select-none">
      <div>
        <h3 className="text-[15px] font-extrabold tracking-[-0.02em] text-slate-900">{title}</h3>
